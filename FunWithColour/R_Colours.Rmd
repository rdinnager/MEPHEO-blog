...
In this post, I am mostly going to play around with ideas about how to get palette of the first kind from an image of something such as our little spider friend above. As already mentioned Jo-Fai Chow has already made a function that uses one technique for doing this. Let's try using that on Mr. Peacock:
...
```{r colour clustering, cache=TRUE, warning=FALSE, message=FALSE}
library(fastcluster)
library(ape)
...
## plot tree!
par(mfrow = c(1,1))
col_tree$tip.label <- rep("-----", length(col_tree$tip.label))
plot(col_tree, show.tip.label=TRUE, type ="fan", no.margin=TRUE, tip.color=rgb(spider.cols.red))
...
ncolours <- 8
col_samp <- t(replicate(10000, as.numeric((rownames(spider.cols.red) %in% sample(rownames(spider.cols.red),ncolours)))))
colnames(col_samp) <- rownames(spider.cols.red)
col_mpd <- mpd(col_samp, as.matrix(col_dists))
col_best <- colnames(col_samp)[col_samp[which.max(col_mpd),]==1]
pie(rep(1, 8), col = col_best, main = "Palette based on Peacock Spider")
```
